test_that("RF100 catalog data exists and has correct structure", {
  ...
  expect_true(all(catalog$collection %in% valid_collections))
...
test_that("search_rf100 works with collection filter", {
  ...
  expect_equal(nrow(biology_results), 9)
  ...
  expect_equal(nrow(medical_results), 8)
...
test_that("search_rf100 with no arguments returns all datasets", {
  ...
  expect_equal(nrow(all_results), 34)
})
...
test_that("list_rf100_datasets works correctly", {
  ...
  expect_equal(length(biology_datasets), 9)
...
test_that("catalog has correct dataset counts per collection", {
  ...
  expect_equal(as.numeric(counts["biology"]), 9)
  expect_equal(as.numeric(counts["medical"]), 8)
  expect_equal(as.numeric(counts["infrared"]), 4)
  expect_equal(as.numeric(counts["damage"]), 3)
  expect_equal(as.numeric(counts["underwater"]), 4)
  expect_equal(as.numeric(counts["document"]), 6)
})
