...
test_that("tests for the FGVC-Aircraft dataset", {
  ...
  expect_type(item$x, "integer")
  expect_length(item$x, 2135040)
  expect_type(item$y, "integer")
  expect_equal(item$y, 1)

  resize_collate_fn <- function(batch) {
    height <- 256
    width <- 256
    xs <- lapply(batch, function(item) {
      img_mat <- matrix(item$x, nrow = height, ncol = width)
      img <- torch::torch_tensor(img_mat, dtype = torch::torch_float())$unsqueeze(1)
      torchvision::transform_resize(img, size = c(224, 224))
    })
    xs <- torch::torch_stack(xs)
    ys <- torch::torch_tensor(
      sapply(batch, function(item) item$y),
