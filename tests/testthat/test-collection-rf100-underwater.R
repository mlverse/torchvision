...
test_that("rf100_underwater_collection handles missing files gracefully", {
  expect_error(
    rf100_underwater_collection(dataset = "pipes", split = "train", root = tempfile(), download = FALSE),
    class = "runtime_error"
  )
})

datasets <- c("pipes", "aquarium", "objects", "coral")

for (ds_name in datasets) {
  test_that(paste0("rf100_underwater_collection loads ", ds_name, " correctly"), {
    skip_if(Sys.getenv("TEST_LARGE_DATASETS", unset = 0) != 1,
            "Skipping test: set TEST_LARGE_DATASETS=1 to enable tests requiring large downloads.")
    ds <- rf100_underwater_collection(dataset = ds_name, split = "train", root = t, download = TRUE)
